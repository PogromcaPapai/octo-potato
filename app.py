...
if switch == "YouTube":
    ...
    st.button("Pobierz", on_click=yt.download, args=(search, replies, amount, omit, sort, title))
elif switch == "Reddit":
    sort = st.checkbox("Sortować według liczby komentarzy (reddit)")
    st.button("Pobierz", on_click=reddit.download, args=(search, replies, amount, omit, sort, title))